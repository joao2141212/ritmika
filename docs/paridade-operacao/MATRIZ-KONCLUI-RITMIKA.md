...
| Operador vê somente tarefas atribuídas | Declarado no app oficial | `EmployeeHome` consulta atribuições por usuário, unidade e papel | Gestor publicou e atribuiu; o mesmo checklist apareceu e foi executado pelo operador QA | Provar setor, unidade, atribuição herdada e ausência para outro operador | parcial |
...
| Busca rápida | Novidade oficial 1.8.0 | Busca textual dentro da home | Busca sem acento localizou título acentuado no gate de produção | Cobrir espaços extras e conjuntos maiores | parcial |
...
| Iniciar execução | Execução guiada oficial | Inicia resposta e fixa `executionId` na URL | Passou no gate recorrente gestor → operação | Manter no gate de release | passou |
| Salvar e retomar | Execução guiada oficial | Salva respostas incrementalmente | Recarregou a URL mantendo 1/1 e 100% no gate de produção | Provar perda de rede, duplicação e sessão expirada | parcial |
| Concluir | Operador avança até finalizar | Valida obrigatórios/evidências e conclui | Status concluído, aviso, histórico e banco em 100% | Provar idempotência por repetição de request | parcial |
...
| Texto/comentário | App oficial declara comentários | Textarea genérica por item | Fixture de capacidades concluiu 3/3 com comentário | Provar exibição detalhada no histórico | parcial |
...
| Data e hora | Contrato autenticado ainda não recapturado | `datetime-local` aceita `date_time` e `datetime` | Fixture de capacidades concluiu 3/3 em produção | Recapturar contrato original autenticado | parcial |
| Foto/vídeo/arquivo | App oficial declara foto; políticas listam foto/vídeo | Upload aceita imagem, PDF e vídeo | Upload obrigatório passou no app e permitiu conclusão | Provar URL assinada, download, vídeo e PDF | parcial |
| Evidência obrigatória | App oficial sustenta comprovação da execução | Bloqueia conclusão sem evidência requerida | Bloqueio, upload, retomada e conclusão passaram no gate de produção | Manter no gate de release | passou |
...
| Perfil e logout | Login é exclusivo a colaboradores | `/app/profile` e logout | Perfil operacional e retorno ao login passaram | Provar bloqueio de back navigation | parcial |
| Bloqueio das rotas de gestão | Implícito pela separação do app do colaborador | `ProtectedRoute audience="manager"` | Operador foi redirecionado ao tentar `/checklists` | Expandir gate para `/`, `/team`, `/configurations` e `/master` | parcial |
