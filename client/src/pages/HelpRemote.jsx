...
const HelpRemote = () => {
    ...
    return (
        <div className="parity-page ritmika-light-mode">
            ...
            <section className="parity-contact-grid"><div className="parity-card"><MessageCircle size={22} /><h2>WhatsApp</h2>{support?.whatsapp_url ? <a href={support.whatsapp_url} target="_blank" rel="noreferrer">Abrir canal de suporte <ExternalLink size={14} /></a> : <div><p>Este canal ainda não foi configurado. Peça a uma pessoa administradora para ativá-lo.</p>{fallbackTutorial && <Link className="parity-link-button" to={fallbackTutorial.route}>Ver guia disponível <PlayCircle size={14} /></Link>}</div>}</div><div className="parity-card"><Mail size={22} /><h2>E-mail</h2>{support?.email ? <a href={`mailto:${support.email}`}>{support.email}</a> : <div><p>O e-mail de suporte ainda não foi configurado. Peça a uma pessoa administradora para adicioná-lo.</p>{fallbackTutorial && <Link className="parity-link-button" to={fallbackTutorial.route}>Ver guia disponível <PlayCircle size={14} /></Link>}</div>}</div></section>
